...
import { AssetMaintenanceForm } from './AssetOperationForms'
...
function getRowAssetName(row: AssetMaintenanceListItemResponse, locale: 'en' | 'ar') {
...
export function AssetMaintenanceListPage() {
  ...
  const [rows, setRows] = useState<AssetMaintenanceListItemResponse[]>([])
  ...
  const loadRows = useCallback(async () => {
    ...
    setError('')
    try {
      const result = await assetService.getAllAssetMaintenance({
  ...
  return (
    <ListPage className="assets-page asset-operations-page asset-maintenance-page">
      <PageHeader
        title={t('assets.maintenanceList.title')}
        description={t('assets.maintenanceList.subtitle')}
        action={
      ...
      <ListCard>
        <ListCardHeader
          title={t('assets.maintenanceList.tableTitle')}
          toolbar={
        ...
        <ListPageStates
          ...
          filterEmptyTitle={t('assets.maintenanceList.empty.filteredTitle')}
          filterEmptyDescription={t('assets.maintenanceList.empty.filteredDescription')}
          showTable={showTable}
          table={
            <DataTable>
              <TableHead>
                <TableRow>
                  ...
                  <Th>{t('assets.maintenance.description')}</Th>
                </TableRow>
              </TableHead>
              <TableBody>
                {rows.map((row) => (
                  <TableRow key={row.id}>
                    ...
                    <Td>{row.description?.trim() || t('assets.common.notAvailable')}</Td>
                  </TableRow>
                ))}
              </TableBody>
            </DataTable>
