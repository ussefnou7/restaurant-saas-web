...
import type { OrderTypeFilter, SalesByPaymentMethodRow, SalesFilterParams } from '../../../types/reports'
...
export function SalesByPaymentMethodReport() {
  ...
  const [rows, setRows] = useState<SalesByPaymentMethodRow[]>([])
  ...
  const loadRows = useCallback(async () => {
    ...
    try {
      ...
      const data = await salesReportService.getSalesByPaymentMethod(filters)
      setRows(data)
    } catch (err) {
  ...
  const dominantMethod = useMemo(() => {
    if (rows.length === 0) return { method: '-', share: '0.0' }
    const top = rows[0]
    const sharePct = parseFloat(top.totalSharePercent) || 0
    const methodLabel = t(`reports.paymentMethod.${top.paymentMethod}` as const)
    return { method: methodLabel, share: sharePct.toFixed(1) }
  }, [rows, t])
  ...
  const filterSentence = useMemo(() => {
    ...
    return locale === 'ar'
      ? `مفلتر حسب الفرع ${branchText}، المستخدم ${userText}، نوع الطلب ${orderTypeText}. مرتب حسب إجمالي المبلغ تنازلياً.`
      : `Filtered by branch ${branchText}, user ${userText}, order type ${orderTypeText}. Sorted by total amount descending.`
          ...
          <div className="report-header-block">
            <div className="report-header-block__top">
              <div className="report-header-block__title-group">
                <h1 className="report-header-block__title">{t('reports.salesByPaymentMethod')}</h1>
                <span className="report-header-block__code-badge">{t('reports.code.salesByPaymentMethod')}</span>
              </div>
            </div>
            <p className="report-header-block__method">{t('reports.salesByPaymentMethod.subtitle')}</p>
          </div>
      ...
      <div className="report-summary-strip">
        ...
        <div className="report-summary-item">
          <span className="report-summary-item__label">
            {t('reports.summary.dominantMethod', { method: dominantMethod.method })}
          </span>
          <span className="report-summary-item__val">
            <span dir="ltr" style={{ unicodeBidi: 'isolate' }}>{dominantMethod.share}%</span>
          </span>
        </div>
      </div>
        ...
        {loadingRows ? (
          ...
          <div className="list-card-content table-wrap">
            <table className="report-ledger-table">
              <thead>
                <tr>
                  <th className="report-cell--text">{t('reports.columns.paymentMethod')}</th>
                  ...
                  <th className="report-cell--numeric">{t('reports.columns.totalShare')}</th>
                </tr>
              </thead>
              <tbody>
                {rows.map((row, idx) => {
                  ...
                  const sharePct = Math.min(100, Math.max(0, parseFloat(row.totalSharePercent) || 0))
                  const methodLabel = t(`reports.paymentMethod.${row.paymentMethod}` as const)
