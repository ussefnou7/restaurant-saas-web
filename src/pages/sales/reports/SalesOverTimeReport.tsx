...
export function SalesOverTimeReport() {
  ...
  const [rows, setRows] = useState<SalesOverTimeRow[]>([])
  ...
  const loadRows = useCallback(async () => {
    ...
    try {
      ...
      const data = await salesReportService.getSalesOverTime(filters)
      setRows(data)
    } catch (err) {
  ...
  const peakDay = useMemo(() => {
    if (rows.length === 0) return { date: '-', value: 0 }
    let maxRow = rows[0]
    ...
    return { date: maxRow.salesDate, value: maxVal }
  }, [rows])
  ...
  const filterSentence = useMemo(() => {
    ...
    return locale === 'ar'
      ? `مفلتر حسب الفرع ${branchText}، المستخدم ${userText}، نوع الطلب ${orderTypeText}. مرتب زمنياً.`
      : `Filtered by branch ${branchText}, user ${userText}, order type ${orderTypeText}. Sorted chronologically.`
  ...
  return (
    <ListPage className="reports-page">
      ...
      <div className="report-summary-strip">
        ...
        <div className="report-summary-item">
          <span className="report-summary-item__label">{t('reports.summary.peakDay')}</span>
          <span className="report-summary-item__val">
            <span dir="ltr" style={{ unicodeBidi: 'isolate' }}>{peakDay.date}</span>{' '}
            <span className="report-summary-item__sub" dir="ltr" style={{ unicodeBidi: 'isolate' }}>
              ({formatMoneyNumber(peakDay.value)} {locale === 'ar' ? 'ج.م' : 'EGP'})
            </span>
          </span>
        </div>
      </div>
