...
interface PurchaseInvoiceMaterialSelectProps {
  value: string
...
export function PurchaseInvoiceMaterialSelect({
  value,
  onChange,
  materials,
  locale,
  ...
  searchPlaceholder,
  ariaLabel,
}: PurchaseInvoiceMaterialSelectProps) {
  ...
  const filtered = useMemo(() => {
    ...
    return materials.filter((m) => {
      const name = getInventoryLocalizedName(m, locale).toLowerCase()
      const code = (m.code ?? '').toLowerCase()
      return name.includes(q) || code.includes(q)
    })
  }, [materials, locale, search])
